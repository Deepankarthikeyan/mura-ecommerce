...
import {
  csvToList,
  formatShopFilterOption,
  type ProductListingFilterOptions,
  type ShopFilterFacet,
...
/** Coerce a product field that may be stored as number or currency string. */
function productFieldAsNumber(fieldPath: string) {
  const stripped = {
    $replaceAll: {
      input: {
        $replaceAll: {
...
function mixedValueInFilter(field: string, values: string[]): Record<string, unknown> {
  const nums = values
    .map((v) => Number(v))
    .filter((n) => Number.isFinite(n));
  const ors: Record<string, unknown>[] = [{ [field]: { $in: values } }];
  if (nums.length) {
    ors.push({ [field]: { $in: nums } });
  }
...
function numericRangeFilter(
  ...
  let lo = min != null && Number.isFinite(min) ? min : null;
  let hi = max != null && Number.isFinite(max) ? max : null;
  ...
  const num = productFieldAsNumber(`$${field}`);
  const parts: Record<string, unknown>[] = [{ $ne: [num, null] }];
  if (lo != null) parts.push({ $gte: [num, lo] });
  if (hi != null) parts.push({ $lte: [num, hi] });
  return { $expr: { $and: parts } };
}

function availabilityFilter(values: string[]): Record<string, unknown> | null {
  const selected = new Set(values);
  const wantIn = selected.has("in-stock");
  const wantOut = selected.has("out-of-stock");
  if (!wantIn && !wantOut) return null;
  if (wantIn && wantOut) return null;

  const stockNum = productFieldAsNumber("$stock");
  if (wantIn) {
    return {
      $or: [
        { stock: { $exists: false } },
  ...
  return {
    $expr: {
      $and: [{ $ne: [stockNum, null] }, { $lte: [stockNum, 0] }],
    },
  };
}

function buildProductListingMongoFilter(
  includeDisabled: boolean,
  options?: ProductListingFilterOptions
): Record<string, unknown> {
  const clauses: Record<string, unknown>[] = [productsListingBaseFilter(includeDisabled)];
  ...
  const category = csvToList(options?.category);
  if (category.length) clauses.push(mixedValueInFilter("category", category));

  const quantity = csvToList(options?.quantity);
  if (quantity.length) clauses.push(mixedValueInFilter("quantity", quantity));

  const tags = csvToList(options?.tags);
  if (tags.length) clauses.push({ tags: { $in: tags } });

  const author = csvToList(options?.author);
  if (author.length) clauses.push(mixedValueInFilter("author", author));

  const ratings = csvToList(options?.ratings);
  if (ratings.length) clauses.push(mixedValueInFilter("ratings", ratings));

  const discounts = csvToList(options?.discountPercentage);
  if (discounts.length) clauses.push(mixedValueInFilter("discountPercentage", discounts));

  const priceRange = numericRangeFilter("price", options?.minPrice, options?.maxPrice);
  if (priceRange) clauses.push(priceRange);

  const availability = availabilityFilter(csvToList(options?.availability));
  if (availability) clauses.push(availability);
...
export async function getProductsListing(
  ...
  try {
    ...
    const filter = buildProductListingMongoFilter(includeDisabled, options);
...
type FacetAggRow = { _id?: unknown; count?: number };

function facetRowsToOptions(
  rows: FacetAggRow[] | undefined,
  key: string,
  extra?: (option: ShopFilterOption) => boolean
): ShopFilterOption[] {
  const options = (rows ?? [])
    .map((row) => {
      const value = String(row._id ?? "").trim();
      return {
        value,
...
function rangeFromStats(
  rows: Array<{ min?: number; max?: number }> | undefined
...
function countFromFacet(rows: Array<{ count?: number }> | undefined): number {
  return Number(rows?.[0]?.count) || 0;
}

/** Distinct shop-filter values from live product properties (excluding name/title). */
export async function getProductFilterFacets(
  includeDisabled: boolean
): Promise<{ facets: ShopFilterFacet[]; totalProducts: number }> {
  try {
    const db = (await getMongoClientPromise()).db();
    const collection = db.collection("products");
    const distinctString = (field: string) => [
      {
        $project: {
          value: {
      ...
      { $match: { value: { $nin: ["", "null", "undefined"] } } },
      { $group: { _id: "$value", count: { $sum: 1 } } },
    ];
    const numericStats = (field: string) => [
      { $project: { n: productFieldAsNumber(`$${field}`) } },
      { $match: { n: { $ne: null, $type: "number" } } },
      { $group: { _id: null, min: { $min: "$n" }, max: { $max: "$n" } } },
    ];
    const numericDistinct = (field: string) => [
      { $project: { n: productFieldAsNumber(`$${field}`) } },
      { $match: { n: { $ne: null, $type: "number" } } },
    ...
    const [agg] = await collection
      .aggregate([
        { $match: productsListingBaseFilter(includeDisabled) },
        {
          $facet: {
            total: [{ $count: "count" }],
            category: distinctString("category"),
            quantity: distinctString("quantity"),
            author: distinctString("author"),
            tags: [
              { $unwind: { path: "$tags", preserveNullAndEmptyArrays: false } },
              {
                $project: {
                  value: {
                    $trim: { input: { $toString: { $ifNull: ["$tags", ""] } } },
                  },
                },
              },
              { $match: { value: { $nin: ["", "null", "undefined"] } } },
            ...
            ratings: numericDistinct("ratings"),
            discount: numericDistinct("discountPercentage"),
            price: numericStats("price"),
            inStock: [
              {
                $match: {
                  $or: [
                    ...
                    { stock: null },
                    { stock: "" },
                    { $expr: { $gt: [productFieldAsNumber("$stock"), 0] } },
                  ],
                },
              },
              { $count: "count" },
            ...
            outStock: [
              {
                $match: {
                  $expr: {
                    $and: [
                      { $ne: [productFieldAsNumber("$stock"), null] },
                      { $lte: [productFieldAsNumber("$stock"), 0] },
                    ],
                  },
                },
              },
    ...
    pushMulti("category", "Category", "category", facetRowsToOptions(agg?.category, "category"));
    pushMulti("quantity", "Quantity", "quantity", facetRowsToOptions(agg?.quantity, "quantity"));
    pushMulti("tags", "Tags", "tag", facetRowsToOptions(agg?.tags, "tags"));
    pushMulti("author", "Author", "author", facetRowsToOptions(agg?.author, "author"));
    pushMulti(
      "ratings",
      "Ratings",
      "rating",
      facetRowsToOptions(agg?.ratings, "ratings").sort((a, b) => Number(b.value) - Number(a.value))
    );
    pushMulti(
      "discountPercentage",
      "Discount",
      "discount",
      facetRowsToOptions(agg?.discount, "discountPercentage", (option) => Number(option.value) > 0)
    );

    const price = rangeFromStats(agg?.price);
    ...
    const inCount = countFromFacet(agg?.inStock);
    const outCount = countFromFacet(agg?.outStock);
    ...
    return {
      facets,
      totalProducts: countFromFacet(agg?.total),
    };
  } catch (error: any) {
    throw new Error("Error fetching product filters: " + error.message);
  }
}
...
/** Update fields on a document in `products` (database from MONGODB_URI). */
...
/** Insert a new product into `products` (database from MONGODB_URI). */
...
/** Single product for storefront; hides soft-deleted. Match productId, slug, urlSlug, or title-derived slug. */
...
export async function sendRegistrationOtpEmail(toEmail: string, otpCode: string) {
  ...
  await transport.sendMail({
    from: muraFromAddress(),
    to: toEmail,
    subject: `Your ${MURA_EMAIL.name} registration code`,
    html: wrapMuraEmail({
      title: `Verify your ${MURA_EMAIL.name} email`,
      preheader: `Your ${MURA_EMAIL.name} verification code is ${otpCode}`,
      bodyHtml: `
        <h2 style="margin:0 0 12px 0;font-family:Georgia,'Times New Roman',serif;font-size:26px;color:${MURA_EMAIL.text};">Verify your email</h2>
        <p style="margin:0 0 8px 0;font-size:15px;line-height:1.6;color:${MURA_EMAIL.muted};">
          Use this code to complete your ${MURA_EMAIL.name} registration and start shopping sale sarees.
        </p>
        <div style="margin:24px 0;padding:18px 20px;text-align:center;background:${MURA_EMAIL.cream};border:1px solid ${MURA_EMAIL.border};border-radius:8px;">
          <div style="font-size:32px;letter-spacing:10px;font-weight:700;color:${MURA_EMAIL.primary};">${otpCode}</div>
        </div>
        <p style="margin:0 0 24px 0;font-size:13px;line-height:1.6;color:${MURA_EMAIL.muted};">
          This code expires in 10 minutes. If you did not create a ${MURA_EMAIL.name} account, you can ignore this email.
        </p>
      `,
    }),
    text: `Your ${MURA_EMAIL.name} registration verification code is: ${otpCode}. It expires in 10 minutes.`,
    attachments: getMuraLogoAttachments(),
  });
}
...
export async function sendPasswordResetOtpEmail(toEmail: string, otpCode: string) {
  ...
  await transport.sendMail({
    from: muraFromAddress(),
    to: toEmail,
    subject: `Your ${MURA_EMAIL.name} password reset code`,
    html: wrapMuraEmail({
      title: `Reset your ${MURA_EMAIL.name} password`,
      preheader: `Your ${MURA_EMAIL.name} password reset code is ${otpCode}`,
      bodyHtml: `
        <h2 style="margin:0 0 12px 0;font-family:Georgia,'Times New Roman',serif;font-size:26px;color:${MURA_EMAIL.text};">Reset your password</h2>
        <p style="margin:0 0 8px 0;font-size:15px;line-height:1.6;color:${MURA_EMAIL.muted};">
          Use this code to reset the password for your ${MURA_EMAIL.name} account.
        </p>
        <div style="margin:24px 0;padding:18px 20px;text-align:center;background:${MURA_EMAIL.cream};border:1px solid ${MURA_EMAIL.border};border-radius:8px;">
          <div style="font-size:32px;letter-spacing:10px;font-weight:700;color:${MURA_EMAIL.primary};">${otpCode}</div>
        </div>
    ...
    text: `Your ${MURA_EMAIL.name} password reset verification code is: ${otpCode}. It expires in 10 minutes.`,
    attachments: getMuraLogoAttachments(),
  });
}
...
export async function sendOrderConfirmationEmail(orderData: any, userEmail: string) {
  try {
    ...
    const itemsList = items.map((item: any) =>
      `<tr>
        <td style="padding: 10px; border: 1px solid ${MURA_EMAIL.border}; color: ${MURA_EMAIL.text};">${item.title}</td>
        <td style="padding: 10px; border: 1px solid ${MURA_EMAIL.border}; text-align: center; color: ${MURA_EMAIL.text};">${item.quantity}</td>
        <td style="padding: 10px; border: 1px solid ${MURA_EMAIL.border}; text-align: right; color: ${MURA_EMAIL.text};">₹${(item.price * item.quantity).toFixed(2)}</td>
      </tr>`
    ).join('');

    const companyAddress = {
      name: MURA_EMAIL.name,
      street: MURA_EMAIL.addressLine1,
      city: MURA_EMAIL.addressLine2,
      phone: MURA_EMAIL.phone,
      email: MURA_EMAIL.email,
    };

    const buildOrderEmailHtml = (orderHeading: string, greetingLine: string) =>
      wrapMuraEmail({
        title: `${orderHeading} · ${MURA_EMAIL.name}`,
        preheader: `${greetingLine} Order #${orderId}`,
        bodyHtml: `
          <h2 style="margin:0 0 10px 0;font-family:Georgia,'Times New Roman',serif;font-size:26px;color:${MURA_EMAIL.primary};">${orderHeading}</h2>
          <p style="margin:0 0 20px 0;font-size:15px;line-height:1.6;color:${MURA_EMAIL.muted};">${greetingLine}</p>

          <div style="background:${MURA_EMAIL.cream};padding:20px;border-radius:8px;margin-bottom:24px;">
            <table role="presentation" style="width:100%;border-collapse:collapse;">
              <tr>
                <td style="padding:8px 0;color:${MURA_EMAIL.muted};font-size:14px;">Order ID:</td>
                <td style="padding:8px 0;color:${MURA_EMAIL.text};font-weight:600;font-size:14px;text-align:right;">${orderId}</td>
              </tr>
              <tr>
                <td style="padding:8px 0;color:${MURA_EMAIL.muted};font-size:14px;">Order Date:</td>
                <td style="padding:8px 0;color:${MURA_EMAIL.text};font-size:14px;text-align:right;">${orderDateDisplay}</td>
              </tr>
              <tr>
                <td style="padding:8px 0;color:${MURA_EMAIL.muted};font-size:14px;">Payment Method:</td>
                <td style="padding:8px 0;color:${MURA_EMAIL.text};font-size:14px;text-align:right;">${paymentMethod}</td>
              </tr>
            </table>
          </div>

          <h3 style="margin:0 0 12px 0;font-family:Georgia,'Times New Roman',serif;font-size:18px;color:${MURA_EMAIL.text};">Sarees Ordered</h3>
          <table role="presentation" style="width:100%;border-collapse:collapse;margin-bottom:20px;">
            <thead>
              <tr style="background:${MURA_EMAIL.primary};color:${MURA_EMAIL.white};">
                <th style="padding:12px;text-align:left;font-size:14px;">Saree</th>
          ...
          <div style="text-align:right;padding:16px;background:${MURA_EMAIL.cream};border-radius:8px;margin-bottom:28px;">
            <span style="color:${MURA_EMAIL.muted};font-size:16px;">Order Total: </span>
            <span style="color:${MURA_EMAIL.primary};font-size:24px;font-weight:700;">₹${total.toFixed(2)}</span>
          </div>

          <h3 style="margin:0 0 12px 0;font-family:Georgia,'Times New Roman',serif;font-size:18px;color:${MURA_EMAIL.text};">Address Details</h3>
          <table role="presentation" style="width:100%;border-collapse:collapse;margin-bottom:24px;">
            <tr>
              <td style="width:50%;vertical-align:top;padding-right:8px;">
                <div style="background:${MURA_EMAIL.cream};padding:18px;border-radius:8px;border-left:4px solid ${MURA_EMAIL.primary};">
                  <h4 style="margin:0 0 10px 0;font-size:12px;letter-spacing:1.2px;text-transform:uppercase;color:${MURA_EMAIL.primary};">From (Dispatch)</h4>
                  <p style="margin:0;color:${MURA_EMAIL.text};font-weight:600;font-size:15px;">${companyAddress.name}</p>
                  <p style="margin:6px 0 0 0;color:${MURA_EMAIL.muted};font-size:13px;line-height:1.6;">
              ...
              <td style="width:50%;vertical-align:top;padding-left:8px;">
                <div style="background:${MURA_EMAIL.beige};padding:18px;border-radius:8px;border-left:4px solid ${MURA_EMAIL.text};">
                  <h4 style="margin:0 0 10px 0;font-size:12px;letter-spacing:1.2px;text-transform:uppercase;color:${MURA_EMAIL.text};">To (Ship To)</h4>
                  <p style="margin:0;color:${MURA_EMAIL.text};font-weight:600;font-size:15px;">${billingInfo.firstName} ${billingInfo.lastName}</p>
                  <p style="margin:6px 0 0 0;color:${MURA_EMAIL.muted};font-size:13px;line-height:1.6;">
          ...
          <p style="margin:0 0 24px 0;font-size:13px;line-height:1.6;color:${MURA_EMAIL.muted};">
            Questions about your saree order? Write to
            <a href="mailto:${MURA_EMAIL.email}" style="color:${MURA_EMAIL.primary};text-decoration:none;">${MURA_EMAIL.email}</a>
            or call ${MURA_EMAIL.phone}.
          </p>
        `,
      });

    const customerGreeting = `Dear ${billingInfo.firstName} ${billingInfo.lastName}, thank you for your ${MURA_EMAIL.name} order.`;
    const staffGreeting = `New ${MURA_EMAIL.name} order from ${billingInfo.firstName} ${billingInfo.lastName}.`;
    ...
    const muraAttachments = getMuraLogoAttachments();

    const userMailOptions = {
      from: muraFromAddress(),
      to: userEmail,
      subject: `Order Placed · ${MURA_EMAIL.name} #${orderId}`,
      html: customerEmailHtml,
      attachments: muraAttachments,
    };
    ...
    if (STAFF_EMAILS.length > 0) {
      const staffEmailPromises = STAFF_EMAILS.map(async (staffEmail) => {
        const staffMailOptions = {
          from: muraFromAddress(),
          to: staffEmail,
          subject: `Order Received · ${MURA_EMAIL.name} #${orderId}`,
          html: staffEmailHtml,
          attachments: muraAttachments,
        };
...
export async function sendOrderActionRequestEmail(params: OrderActionEmailParams) {
  ...
  const selectedProductsHtml =
    selectedProducts && selectedProducts.length > 0
      ? `
        <h3 style="margin:0 0 8px 0;font-family:Georgia,'Times New Roman',serif;font-size:16px;color:${MURA_EMAIL.text};">Selected sarees</h3>
        <ul style="margin:0 0 16px 0;padding-left:20px;color:${MURA_EMAIL.muted};font-size:14px;line-height:1.6;">
          ${selectedProducts
            .map(
              (product) =>
  ...
  const html = wrapMuraEmail({
    title: `Order ${actionLabel} Request · ${MURA_EMAIL.name}`,
    preheader: `${displayName} submitted a ${actionLabel.toLowerCase()} request for order ${orderNo}`,
    bodyHtml: `
      <h2 style="margin:0 0 12px 0;font-family:Georgia,'Times New Roman',serif;font-size:24px;color:${MURA_EMAIL.primary};">Order ${actionLabel} Request</h2>
      <p style="margin:0 0 16px 0;color:${MURA_EMAIL.muted};font-size:15px;line-height:1.6;">
        A customer has submitted a <strong>${actionLabel.toLowerCase()}</strong> request for their ${MURA_EMAIL.name} order.
      </p>
      <table role="presentation" style="width:100%;border-collapse:collapse;margin-bottom:20px;">
        <tr>
          <td style="padding:8px 0;color:${MURA_EMAIL.muted};font-size:14px;width:140px;">Order:</td>
          <td style="padding:8px 0;color:${MURA_EMAIL.text};font-weight:600;font-size:14px;">${orderNo}</td>
        </tr>
        <tr>
          <td style="padding:8px 0;color:${MURA_EMAIL.muted};font-size:14px;">Customer:</td>
          <td style="padding:8px 0;color:${MURA_EMAIL.text};font-size:14px;">${displayName}</td>
        </tr>
        <tr>
          <td style="padding:8px 0;color:${MURA_EMAIL.muted};font-size:14px;">Email:</td>
          <td style="padding:8px 0;color:${MURA_EMAIL.text};font-size:14px;">
            <a href="mailto:${userEmail}" style="color:${MURA_EMAIL.primary};text-decoration:none;">${userEmail}</a>
          </td>
        </tr>
        <tr>
          <td style="padding:8px 0;color:${MURA_EMAIL.muted};font-size:14px;">Request type:</td>
          <td style="padding:8px 0;color:${MURA_EMAIL.text};font-size:14px;">${actionLabel}</td>
        </tr>
      </table>
      ${selectedProductsHtml}
      <h3 style="margin:0 0 8px 0;font-family:Georgia,'Times New Roman',serif;font-size:16px;color:${MURA_EMAIL.text};">Reason</h3>
      <div style="background:${MURA_EMAIL.cream};padding:16px;border-radius:8px;border-left:4px solid ${MURA_EMAIL.primary};margin-bottom:16px;">
        <p style="margin:0;color:${MURA_EMAIL.muted};font-size:14px;line-height:1.6;white-space:pre-wrap;">${escapeHtml(reason)}</p>
      </div>
      ${attachment ? `<p style="margin:0 0 16px 0;color:${MURA_EMAIL.muted};font-size:13px;">Supporting document is attached to this email.</p>` : ''}
    `,
  });
  ...
  try {
    ...
    const staffEmailPromises = STAFF_EMAILS.map(async (staffEmail) => {
      const result = await transport.sendMail({
        ...
        replyTo: userEmail,
        to: staffEmail,
        subject: `Order ${actionLabel} Request · ${MURA_EMAIL.name} ${orderNo}`,
        html,
        text: `${MURA_EMAIL.name} Order ${actionLabel} Request\n\nOrder: ${orderNo}\nCustomer: ${displayName}\nEmail: ${userEmail}${selectedProductsText}\n\nReason:\n${reason}`,
        attachments: [
          ...getMuraLogoAttachments(),
          ...(attachment
            ? [
...
export async function sendOrderStatusUpdateEmail(
  ...
  const html = wrapMuraEmail({
    title: `Order Status Updated · ${MURA_EMAIL.name}`,
    preheader: `Your ${MURA_EMAIL.name} order ${orderId} is now ${statusDisplay}`,
    bodyHtml: `
      <h2 style="margin:0 0 10px 0;font-family:Georgia,'Times New Roman',serif;font-size:26px;color:${MURA_EMAIL.primary};">Order Status Updated</h2>
      <p style="margin:0 0 20px 0;font-size:15px;line-height:1.6;color:${MURA_EMAIL.muted};">
        Dear ${customerName}, your ${MURA_EMAIL.name} saree order has a new update.
      </p>
      <div style="background:${MURA_EMAIL.cream};padding:20px;border-radius:8px;margin-bottom:20px;">
        <table role="presentation" style="width:100%;border-collapse:collapse;">
          <tr>
            <td style="padding:8px 0;color:${MURA_EMAIL.muted};font-size:14px;">Order ID:</td>
            <td style="padding:8px 0;color:${MURA_EMAIL.text};font-weight:600;font-size:14px;text-align:right;">${orderId}</td>
          ...
          <tr>
            <td style="padding:8px 0;color:${MURA_EMAIL.muted};font-size:14px;">New Status:</td>
            <td style="padding:8px 0;color:${MURA_EMAIL.primary};font-weight:700;font-size:16px;text-align:right;">${statusDisplay}</td>
          </tr>
        </table>
      </div>
      <div style="text-align:right;padding:16px;background:${MURA_EMAIL.beige};border-radius:8px;margin-bottom:20px;">
        <span style="color:${MURA_EMAIL.muted};font-size:16px;">Order Total: </span>
        <span style="color:${MURA_EMAIL.primary};font-size:24px;font-weight:700;">₹${total.toFixed(2)}</span>
      </div>
      <p style="margin:0 0 16px 0;color:${MURA_EMAIL.muted};font-size:14px;line-height:1.6;">
  ...
  const result = await transport.sendMail({
    from: muraFromAddress(),
    to: userEmail,
    subject: `Order Status Updated · ${statusDisplay} · ${MURA_EMAIL.name} #${orderId}`,
    html,
    text: `Dear ${customerName},\n\nYour ${MURA_EMAIL.name} order ${orderId} status has been updated to: ${statusDisplay}.\n\nOrder total: ₹${total.toFixed(2)}\n\nThank you for shopping with ${MURA_EMAIL.name}.`,
    attachments: getMuraLogoAttachments(),
  });
...
const WEBSITE_SETTINGS_COLLECTION = "website_settings";
const WEBSITE_COLORS_KEY = "colors";

function colorsFromDoc(doc: { colors?: unknown } | null): Record<string, unknown> {
...
function themeColorsKey(theme: string) {
  return `colors_${theme}`;
}

export async function getWebsiteColorSettings(
  theme?: string,
  ...
  try {
    ...
    if (theme) {
      const themed = await db
        .collection(WEBSITE_SETTINGS_COLLECTION)
        .findOne({ key: themeColorsKey(theme) });
      const fromTheme = colorsFromDoc(themed);
      if (Object.keys(fromTheme).length > 0) return fromTheme;
      if (theme === "ayurvedha") {
        const legacy = await db
          .collection(WEBSITE_SETTINGS_COLLECTION)
          .findOne({ key: WEBSITE_COLORS_KEY });
        return colorsFromDoc(legacy);
      }
      return {};
    }
    const doc = await db.collection(WEBSITE_SETTINGS_COLLECTION).findOne({ key: WEBSITE_COLORS_KEY });
    return colorsFromDoc(doc);
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : "Fetch failed";
    throw new Error("Error fetching website colors: " + msg);
  }
}

export async function upsertWebsiteColorSettings(
  colors: Record<string, string>,
  theme?: string,
) {
  try {
    const db = (await getMongoClientPromise()).db();
    const key = theme ? themeColorsKey(theme) : WEBSITE_COLORS_KEY;
    const result = await db.collection(WEBSITE_SETTINGS_COLLECTION).updateOne(
      { key },
      {
        $set: {
          key,
    ...
    if (theme === "ayurvedha") {
      await db.collection(WEBSITE_SETTINGS_COLLECTION).updateOne(
        { key: WEBSITE_COLORS_KEY },
        {
          $set: {
            key: WEBSITE_COLORS_KEY,
            colors,
    ...
    const msg = error instanceof Error ? error.message : "Save failed";
    throw new Error("Error saving website colors: " + msg);
  }
}

const HOMEPAGE_SECTIONS_KEY = "homepage_sections";

export async function getHomepageSectionSettings(): Promise<Record<string, unknown> | null> {
  try {
    const db = (await getMongoClientPromise()).db();
    const doc = await db.collection(WEBSITE_SETTINGS_COLLECTION).findOne({ key: HOMEPAGE_SECTIONS_KEY });
    if (!doc) return null;
    return {
      carouselEnabled: doc.carouselEnabled,
      carouselSlides: doc.carouselSlides,
      discountsEnabled: doc.discountsEnabled,
      discountCards: doc.discountCards,
      categoryProductsEnabled: doc.categoryProductsEnabled,
      categoryProducts: doc.categoryProducts,
      dealsOfTheDayEnabled: doc.dealsOfTheDayEnabled,
      dealsOfTheDay: doc.dealsOfTheDay,
      allProductsEnabled: doc.allProductsEnabled,
      allProducts: doc.allProducts,
      newsletterEnabled: doc.newsletterEnabled,
      newsletter: doc.newsletter,
      recommendationsEnabled: doc.recommendationsEnabled,
      recommendations: doc.recommendations,
      sectionOrder: doc.sectionOrder,
    };
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : "Fetch failed";
    throw new Error("Error fetching homepage sections: " + msg);
  }
}

export async function upsertHomepageSectionSettings(payload: {
  ...
  try {
    const db = (await getMongoClientPromise()).db();
    return db.collection(WEBSITE_SETTINGS_COLLECTION).updateOne(
      { key: HOMEPAGE_SECTIONS_KEY },
      {
        $set: {
          key: HOMEPAGE_SECTIONS_KEY,
          carouselEnabled: payload.carouselEnabled,
          carouselSlides: payload.carouselSlides,
          discountsEnabled: payload.discountsEnabled,
          discountCards: payload.discountCards,
          categoryProductsEnabled: payload.categoryProductsEnabled,
          categoryProducts: payload.categoryProducts,
          dealsOfTheDayEnabled: payload.dealsOfTheDayEnabled,
          dealsOfTheDay: payload.dealsOfTheDay,
          allProductsEnabled: payload.allProductsEnabled,
    ...
    const msg = error instanceof Error ? error.message : "Save failed";
    throw new Error("Error saving homepage sections: " + msg);
  }
}

const NEWSLETTER_SUBSCRIBERS_COLLECTION = "newsletter_subscribers";

export async function addNewsletterSubscriber(email: string): Promise<{ created: boolean }> {
  try {
    const db = (await getMongoClientPromise()).db();
    const collection = db.collection(NEWSLETTER_SUBSCRIBERS_COLLECTION);
    ...
    const msg = error instanceof Error ? error.message : "Save failed";
    throw new Error("Error saving newsletter subscriber: " + msg);
  }
}

const WEBSITE_THEME_KEY = "theme";

export async function getWebsiteThemeSetting(): Promise<string | null> {
  try {
    const db = (await getMongoClientPromise()).db();
    const doc = await db.collection(WEBSITE_SETTINGS_COLLECTION).findOne({ key: WEBSITE_THEME_KEY });
    if (!doc || typeof doc.theme !== "string") return null;
    return doc.theme;
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : "Fetch failed";
    throw new Error("Error fetching website theme: " + msg);
  }
}

export async function upsertWebsiteThemeSetting(theme: string) {
  try {
    const db = (await getMongoClientPromise()).db();
    return db.collection(WEBSITE_SETTINGS_COLLECTION).updateOne(
      { key: WEBSITE_THEME_KEY },
      {
        $set: {
          key: WEBSITE_THEME_KEY,
          theme,
    ...
    const msg = error instanceof Error ? error.message : "Save failed";
    throw new Error("Error saving website theme: " + msg);
  }
}