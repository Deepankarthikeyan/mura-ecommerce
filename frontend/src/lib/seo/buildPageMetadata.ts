...
import { SITE_DESCRIPTION, SITE_LOGO, SITE_NAME, SITE_URL, isLegacyBrandText, toPublicAssetUrl } from "../brand";
...
export async function resolvePageSeoConfig(
  ...
  const savedRaw = pagesMap[normalized];
  const saved = isLegacyBrandText(savedRaw)
    ? null
    : parsePageSeoConfig(savedRaw);
  ...
  if (options?.templatePath) {
    ...
    const templateRaw = pagesMap[templateNormalized];
    const templateSaved = isLegacyBrandText(templateRaw)
      ? null
      : parsePageSeoConfig(templateRaw);
...
function resolveImageUrl(image: string | undefined, fallback?: string): string | undefined {
  ...
  return toPublicAssetUrl(raw, fallback || SITE_LOGO);
}
...
export async function buildPageMetadataForPath(
  ...
  const siteName =
    ...
    (isLegacyBrandText(pageSeo.og?.siteName) ? "" : pageSeo.og?.siteName?.trim()) ||
    (isLegacyBrandText(globalBasic.title) ? "" : globalBasic.title?.trim()) ||
    SITE_NAME;
  ...
  const metadata: Metadata = {
    ...
    openGraph: {
      ...
      type: ogType,
      url: toPublicAssetUrl(applySeoTemplate(pageSeo.og?.url, vars) || canonical, "/"),
      siteName: isLegacyBrandText(pageSeo.og?.siteName) ? siteName : pageSeo.og?.siteName || siteName,
      locale: pageSeo.og?.locale || "en_IN",
      images: [
        { url: "/opengraph-image", width: 1200, height: 630, alt: resolvedTitle, type: "image/png" },
        ...(ogImage ? [{ url: ogImage, alt: resolvedTitle }] : []),
      ],
    },
    twitter: {
      card:
      ...
      images: ["/twitter-image", ...(twitterImage ? [twitterImage] : [])],
    },
  };
...
export function productVariables(
  ...
  return {
    ...
    category: String(product?.category ?? "Sarees").trim(),
