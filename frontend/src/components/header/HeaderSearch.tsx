...
function HeaderMenuLinks({ isAuthenticated, user, onLoginClick }: { isAuthenticated: boolean; user: any; onLoginClick: () => void }) {
    return (
        <div className="header-menu-links flex items-center gap-4 text-lg">
            <Link href="/about" className="theme-link-hover transition-colors">About Us</Link>
            {isAuthenticated && user ? (
                <Link href={getDashboardPrefix(user.userType)} className="theme-link-hover transition-colors">
                    Hi, {user.username}
                </Link>
            ) : (
                <button
                    onClick={onLoginClick}
                    className="theme-link-hover transition-colors"
            ...
            <Link href="/wishlist" className="theme-link-hover transition-colors">Wishlist</Link>
            <Link href="/order-tracking" className="theme-link-hover transition-colors">Order Tracking</Link>
        </div>
    );
}
