...
const ProductTable = () => {
  ...
  useEffect(() => {
    ...
    if (savedProducts) {
      ...
      const defaultProducts = ProductData.map((datum) => {
        return {
          id: datum?.id,
          name: datum?.title,
          productNo: datum?.productId,
          category: datum?.category,
          price: `$${datum?.price}`,
          date: datum?.publishedDate || new Date().toISOString().split('T')[0],
          stock: datum?.stock,
          image: datum?.image
        }
      })
      ...
      cell: row => (
        <div style={{ position: 'relative' }}>
          <i
            ...
            style={{ cursor: 'pointer', fontSize: 18, color: '#525252' }}
          />
