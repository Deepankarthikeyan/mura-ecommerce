...
import { parseProductFiltersFromSearchParams } from "@/lib/shopProductFilters";
import {
...
export async function GET(request: Request) {
  try {
    ...
    const facetsOnly =
      searchParams.get("facets") === "true" ||
      searchParams.get("facets") === "1";
    ...
    if (facetsOnly) {
      const facets = await getProductFilterFacets(includeDisabled);
      return NextResponse.json({ success: true, body: facets });
    }
    ...
    const { items, total } = await getProductsListing(includeDisabled, filters, pagination);
    if (!pagination) {
      return NextResponse.json({ success: true, body: items });
    }
